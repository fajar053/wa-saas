...
import { 
  getOAuth2Client, 
  ensureProductSpreadsheet, 
  uploadImageToDriveAndCleanup, 
  appendProductToSheet 
} from "./services/googleService.js";

// Endpoint Dapatkan Link Login Google
app.get("/api/auth/google/url", verifyToken, (req, res) => {
...
app.get("/api/auth/google/callback", async (req, res) => {
  try {
    ...
    if (tokens.refresh_token) {
      const spreadsheetId = await ensureProductSpreadsheet(tokens.refresh_token);
    ...
    res.send(`
      <script>
        alert("Berhasil terhubung ke Google Drive & Spreadsheet!");
        window.location.href = "/products.html";
      </script>
    `);
  } catch (err) {
    res.status(500).send("Gagal mengoneksikan akun Google: " + err.message);
  }
});

// Endpoint Tambah Produk (Di-Gating Wajib Konek Google)
app.post("/api/products", verifyToken, uploadProductMedia.single("imageFile"), async (req, res) => {
  try {
    const user = await User.findById(req.user.userId);
    if (!user) return res.status(404).json({ success: false, message: "User tidak ditemukan!" });

    // Proteksi: Wajib konek Google sebelum upload
    if (!user.googleRefreshToken || !user.googleSpreadsheetId) {
      ...
      return res.status(403).json({
        success: false,
        message: "Akses ditolak! Anda harus mengoneksikan akun Google Drive & Spreadsheet terlebih dahulu."
      });
    }
    ...
    // Upload langsung ke Google Drive & hapus file lokal
    if (req.file) {
      const fullPath = path.join(__dirname, req.file.path);
      driveImageUrl = await uploadImageToDriveAndCleanup(
    ...
    // Rekap ke Google Spreadsheet
    await appendProductToSheet(user.googleRefreshToken, user.googleSpreadsheetId, {
      name,
      price: Number(price),
      description: description || "",
    ...
    // Simpan referensi ringan di MongoDB (tanpa simpan file fisik di server)
    const newProduct = await Product.create({
      userId: user._id,
      name,
      price: Number(price),
    ...
    res.json({
      success: true,
      message: "Produk berhasil tersimpan di Google Drive & Spreadsheet!",
      data: newProduct
    });
  } catch (err) {
    if (req.file && fs.existsSync(req.file.path)) fs.unlinkSync(req.file.path);
