...
app.post("/api/generate-prompt", verifyToken, async (req, res) => {
  try {
    ...
    if (mode === "very_detailed" && user.plan !== "premium") {
      return res.status(403).json({ success: false, message: "Fitur khusus Premium!" });
    }

    if (!promptText || !promptText.trim()) {
      return res.status(400).json({ success: false, message: "Ketikkan instruksi singkat terlebih dahulu!" });
    }
    ...
    const messages = [
      { role: "system", content: "Kamu adalah AI Prompt Engineer. Buatkan System Prompt WhatsApp komprehensif tanpa teks basa-basi." },
